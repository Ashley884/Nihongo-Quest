...
const state = { topics: [], questions: [], currentTopic: null, currentIndex: 0, selected: null, score: 0, adminUser: null, adminTopics: [], adminQuestions: [], editingTopic: null, editingQuestion: null };
...
function layout(content, admin=false) {
  app.innerHTML = `<header class="site-header"><a class="brand" href="/"><span class="brand-mark">N</span><span>NIHONGO QUEST</span></a><div class="header-actions"><button class="ghost" id="menuBtn">☰</button>${admin ? '<button class="admin-pill" id="logoutBtn">Log out</button>' : '<a class="admin-pill" href="/admin">Admin</a>'}</div></header>${content}<div id="toast" class="toast"></div>`;
...
async function loadTopics(){
  ...
  state.topics = error ? fallbackTopics : (data || []);
}
...
function renderQuestion(){
  const q=state.questions[state.currentIndex], total=state.questions.length, pct=Math.round((state.currentIndex/total)*100);
...
function nextQuestion(){
  ...
  if(state.selected===Number(state.questions[state.currentIndex].correct_index)) state.score++;
...
async function renderAdmin(){
  ...
  const {data: qs,error:qErr}=await supabase.from('questions').select('*').order('sort_order'); if(qErr){toast(qErr.message,true);return;} state.adminQuestions=qs||[];
  layout(`<main class="admin-shell wide"><div class="admin-title"><div><div class="eyebrow dark">PRIVATE ADMIN</div><h1>Quiz Manager</h1><p>Manage the 40 topic links and every quiz question.</p></div><button id="newTopic" class="primary">+ New Topic</button></div><div class="admin-layout"><aside class="admin-topics"><h3>Topics</h3><div id="adminTopicList"></div></aside><section class="manager" id="manager"></section></div></main>`,true);
